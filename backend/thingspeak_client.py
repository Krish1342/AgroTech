...
MODEL_FIELD_STR = os.getenv('THINGSPEAK_MODEL_FIELDS', '1,2,3,4,5,6')
MODEL_FIELDS = [int(x) for x in MODEL_FIELD_STR.split(',') if x.strip().isdigit()]
WEATHER_FIELD_STR = os.getenv('THINGSPEAK_WEATHER_FIELDS', '7,8,9,10')
WEATHER_FIELDS = [int(x) for x in WEATHER_FIELD_STR.split(',') if x.strip().isdigit()]
THINGSPEAK_RECOMMENDATION_CHANNEL_ID = os.getenv('THINGSPEAK_RECOMMENDATION_CHANNEL_ID')
THINGSPEAK_RECOMMENDATION_WRITE_API_KEY = os.getenv('THINGSPEAK_RECOMMENDATION_WRITE_API_KEY', '')
...
def get_latest_feed(channel_id: Optional[str]=None, read_key: Optional[str]=None, fields: Optional[List[int]]=None) -> Dict[int, Optional[float]]:
    """
    Returns a dict mapping field number -> numeric value (or None).
    Example: {1: 10.0, 2: 5.0, 3: 7.0, 4: 30.0, 5: 6.5, 6: 25.0}
    """
...
def get_model_input_dict() -> Dict[str, Any]:
    ...
    Keys: 'N','P','K','Moisture','pH','temp' plus optional 'weather' subdict.
    """
    feed = get_latest_feed()
    mapping = {}
    # map MODEL_FIELDS in order to semantic names
    names = ['N','P','K','Moisture','pH','temp']
    ...
    # fetch weather fields if available
    weather = {}
    wf = get_latest_feed(fields=WEATHER_FIELDS)
    if any(v is not None for v in wf.values()):
        # assume order [temp, humidity, wind, condition_text]
        weather['temp'] = wf.get(WEATHER_FIELDS[0]) if len(WEATHER_FIELDS) >= 1 else None
        weather['humidity'] = wf.get(WEATHER_FIELDS[1]) if len(WEATHER_FIELDS) >= 2 else None
        weather['wind'] = wf.get(WEATHER_FIELDS[2]) if len(WEATHER_FIELDS) >= 3 else None
        # condition_text requires separate read via channel feed text (ThingSpeak stores text in fieldN too)
        # We attempt to read condition as numeric fallback would be None; leave None if not numeric
...
def get_recommendations(write_back: bool = False) -> Dict[str, Any]:
    ...
    if write_back:
        if THINGSPEAK_RECOMMENDATION_CHANNEL_ID and THINGSPEAK_RECOMMENDATION_WRITE_API_KEY:
            try:
                params = {
                ...
                r = requests.post('https://api.thingspeak.com/update', params=params, timeout=10)
                r.raise_for_status()
            except Exception as e:
                print(f'Failed to write recommendations to ThingSpeak: {e}')
        else:
            print('Recommendation write-back skipped: recommendation channel or write key not configured.')
